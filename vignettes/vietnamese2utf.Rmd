---
title: "Overview of the vietnamese2utf package"
...
output:
  ...
  html_document:
    ...
    # df_print: kable
vignette: >
  %\VignetteIndexEntry{Overview of the vietnamese2utf package} 
...
```{r eval = FALSE}
install.packages("vietnamese2utf")
```
...
```{r}
test <- decodeVN(string_garbled)
# test
# print(test)
# summary(test)
# utf8::utf8_print(test)
test2 <- knitr::asis_output(test)
test2
knitr::asis_output(decodeVN(string_garbled))
```
...
There is also an argument `to`, which by default is set to `to = \"Unicode\"`. If you want output that simulates data encoded in one of the supported Vietnamese encodings, set argument `to`, e.g.
...
```
Note: when printing a data frame the Unicode characters are not displayed properly, which is a limitation of the print method for data frames in R. It shows correctly when printing the values inside the columns, meaning the data are correct:

```{r}
head(vn_samples$Unicode$Province_city)
```
It also shows correctly when using `View` (not shown here, works in interactive sessions):
...
The other entries in `vn_samples` show garbled text. Here's the example in TCVN3 encoding:
...
Again, we can chose to return characters without accents:
```{r}
...
We can also use the `from` and `to` arguments for other conversions between other encodings.
...
For `sf` objects, the workflow can be as follows (data are not included, code is for illustration only)
...
st_set_geometry(decodeVN, sf_object_geometry)  # assign geometry column to decoded data frame (to make it spatial again)
```
